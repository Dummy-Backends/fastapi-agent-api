...
def call_simulated_agent_brain(task: Dict[str, Any], trace: List[Dict[str, Any]]) -> str:
    ...
    Ensures that mock/offline runs execute a true feedback loop rather than a hardcoded pipeline.
    """
    task_id = task["id"]
    ...
    if not has_get_task:
        ...
        return f"INVESTIGATION_COMPLETE: Mock investigation completed for task {task_id}. Retrieved details, PR events, blockers, and semantic relations."
...
"""
        ...
        for step in range(max_steps):
            ...
            # Fetch decision: Use dynamic simulated brain in mock mode or when API keys are absent
            if LLM_PROVIDER == "mock" or not (GEMINI_API_KEY or OPENAI_API_KEY):
                response = call_simulated_agent_brain(task, self.trace)
            else:
                response = call_llm(system_prompt, current_prompt, temperature=0.3)
                if response == "MOCK_LLM_RESPONSE":
                    response = call_simulated_agent_brain(task, self.trace)
